...
procedure exercicio1_analisador is
   ...
   begin
      Add ("if"); Add ("then"); Add ("else"); Add ("elsif");
      Add ("begin"); Add ("procedure"); Add ("is"); Add ("end");
      Add ("loop"); Add ("with"); Add ("use"); Add ("put");
      Add ("put_line"); Add ("get"); Add ("return"); Add ("type");
   end Carregar_Dicionario;

   -- Lógica para classificar números (Inteiros/Reais e Positivos/Negativos)
   procedure Classificar_Token_Numerico (S : String; Eh_Num : out Boolean; Msg : out Unbounded_String) is
   ...
   begin
      ...
      if S (S'First) = '-' then
         if S'Length = 1 then return; end if;
         Eh_Neg := True;
         Inicio := S'First + 1;
      end if;
      ...
      for I in Inicio .. S'Last loop
         if S (I) = '.' then
            if Tem_Ponto then return; end if; -- Segundo ponto é erro
            Tem_Ponto := True;
      ...
      Eh_Num := True;
      if Tem_Ponto then
         Msg := To_Unbounded_String (if Eh_Neg then "Numero Real Negativo (Float)" else "Numero Real (Float)");
      else
         Msg := To_Unbounded_String (if Eh_Neg then "Numero Inteiro Negativo (Integer)" else "Numero Inteiro (Integer)");
      end if;
      ...
      -- PASSO 2: Verificar se é Número (Inteiro ou Real)
         ...
         -- PASSO 3: Verificar se é Identificador (As 4 Regras)
            ...
            if Tam > 0 and then Is_Letter (S (1)) and then S (Tam) /= '_' 
               ...
               and then (for all I in S'Range => Is_Alphanumeric (S (I)) or S (I) = '_')
               -- Regra 4: Não ter "__"
               and then (for all I in S'First .. S'Last - 1 => not (S (I) = '_' and S (I + 1) = '_'))
            then
